...
impl Config {
    pub fn from_env() -> Result<Self> {
        ...
        // Required: token
        let github_token = if let Ok(v) = std::env::var("RD_GITHUB_TOKEN") {
            v
            ...
            anyhow::bail!("Set RD_GITHUB_TOKEN or run `prctrl config init`");
        };

        // Required: username
        let github_username = if let Ok(v) = std::env::var("RD_GITHUB_USERNAME") {
            v
            ...
            anyhow::bail!("Set RD_GITHUB_USERNAME or run `prctrl config init`");
        };

        // Required: org
        let github_org = if let Ok(v) = std::env::var("RD_GITHUB_ORG") {
            v
            ...
            anyhow::bail!("Set RD_GITHUB_ORG or run `prctrl config init`");
        };

        // Optional: repos
        let github_repos = if let Ok(v) = std::env::var("RD_GITHUB_REPOS") {
        ...
        // Optional: teams
        let github_teams = if let Ok(v) = std::env::var("RD_GITHUB_TEAMS") {
        ...
        // Optional: crew
        let crew_members = std::env::var("RD_CREW_MEMBERS")
            .or_else(|_| std::env::var("CREW_MEMBERS"))
            .map(|s| s.split(',').map(|s| s.trim().to_string()).filter(|s| !s.is_empty()).collect())
            .unwrap_or_default();

        let anthropic_api_key = std::env::var("RD_ANTHROPIC_API_KEY")
            .ok()
